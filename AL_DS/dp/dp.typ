...
= 认识动态规划
== 抽象概念
动态规划是一种算法设计思想，其核心是记录子问题答案，避免反复计算重叠子问题，从而降低蛮力枚举算法的计算时间，俗称“空间换时间”，常用于解决只需要知道结果，而不需要知道具体中间过程的问题，例如：最优化问题、计数问题、存在性问题，因为动态规划并不关心得到问题答案的具体方案，而是关心每个问题的答案与它子问题的答案之间有什么联系

在动态规划的语境下，问题的答案叫做*状态*，通过递推式描述的、答案之间的联系叫做*状态转移方程*，动态规划的实现过程就是利用初始状态和状态转移方程进行自底向上的递推，并达到目标状态，这一过程显然隐含了对枚举树的剪枝操作，使得动态规划算法通常能够在多项式时间内解决问题

一个问题只有在满足以下条件时，才能够利用动态规划解决
1. 可以被分解为相同且规模更小的子问题
2. 具有最优子结构，即，问题的最优解可以由其子问题的最优解得到
3. 子问题具有重叠性，即，存在大量需要被重复计算的子问题
4. 状态系统具有无后效性，即，在已知当前状态的前提下，未来某个状态只与当前状态，以及该未来状态所处阶段有关，与过去状态无关，也就是说，过去状态和未来状态不会相互影响

  动态规划算法在递推时要求按顺序且不重复地计算每一个状态，因此必须满足无后效性
== 具体例子
问题：求斐波那契数列的第$n$项

根据斐波那契数列的递推式$f(n)=f(n-1)+f(n-2), f(1)=1, f(2)=1$，可以写出下列递归求解代码
```cpp
int f(int n) {
...
应用到这里可以得到：数列中，最长不升子序列的数量等于最长上升子序列的长度
=== 二维线性DP问题
如果某问题的状态对应的`dp`数组是二维数组，那么称该问题为二维线性DP问题
==== 最长公共子序列问题
===== 问题描述
===== 状态分析
状态`dp[i][j]`表示：序列$a$的前`i`个字符与序列$b$的前$j$个字符的最长公共子序列长度
...
===== 状态分析
状态`dp[i][j]`表示：考虑`i`号及其之前的所有物品，使用不超过`j`个单位的背包负重，能取得的最大价值
...
- 空间上的优化
  ...
  由状态转移方程可知，求`dp[i][j]`只需要知道它的上一行是什么，利用滚动数组的技巧，可以先将第`i - 1`行复制到第`i`行，再在第`i`行上横向进行状态转移，这样就不再需要`[i]`这个维度，从而将二维数组优化为一维数组，优化后的状态为`dp[j]`，表示使用不超过`j`个单位的背包负重，能取得的最大价值

  优化后，不选的情况下，状态转移方程为`dp[j] = dp[j]`，其实就是不需要改变，这是因为在递推之前，`dp[j]`已经继承了它的上一行（二维`dp`数组中的`dp[i - 1][j]`的数值）；选的情况下，状态转移方程为`dp[j] = dp[j - w[i]] + v[i]`，此时要注意遍历顺序，把`j`看作数组中的一个指针，`j`扫过的地方就是已更新的值，即当前行的值，未扫过的地方就是上一行的值，显然，在遍历背包容量时必须倒序遍历，到`j < w[i]`时停止，因为`dp[j]`总是由它上一行的状态转移过来，如果正序遍历，就变成了`dp[j]`由当前行的状态转移，不符合状态转移方程，用放物品的方式理解，正序遍历就变成了多次放入物品，与0-1背包问题的限制不一致
==== 完全背包问题
===== 问题描述
...
在完全背包问题中，对于`i`号物品考虑的是选$0$个与选$>=1$个两种情况，选$0$个相当于没有考虑`i`号物品，状态转移方程是`dp[i][j] = dp[i - 1][j]`；选$>=1$个的情况下，如果`i`号物品不是第一次被选，那么在此之前`i`号已被考虑，状态转移方程为`dp[i][j] = dp[i][j - w[i]] + v[i]`，

如果是第一次选，那么应该让`dp[i][j - w[i]] + v[i]`与`dp[i - 1][j - w[i]] + v[i]`是同一个值，因为在第一次选之前背包容量显然处于`j < w[i]`的情况
=== $n$维线性DP问题$(n > 2)$
