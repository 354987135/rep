...
在动态规划的语境下，问题本身叫做*阶段*，问题的答案叫做*状态*，通过递推式描述的、答案之间的联系叫做*状态转移方程*，动态规划的实现过程就是利用初始状态和状态转移方程进行自底向上的递推，并达到目标状态，这一过程自然包含了对枚举树大量无效分支的剪枝操作，使得动态规划算法通常能够在多项式时间内解决问题
...
3. 状态系统具有无后效性：在已知当前状态的前提下，未来某个状态只与当前状态及该未来状态所处阶段有关，也就是说，过去状态和未来状态不会相互影响

  递推时要求按顺序且不重复地计算每一个状态，必须满足无后效性
