...
= 常见的动态规划问题模型
== 线性DP
若某DP问题的状态转移方程仅包含线性运算（加法与数乘运算），则称之为线性DP问题，这些问题根据各自的特征不同又可以细分为一些特殊模型
=== 一维线性DP
...
==== 状态分析
令`dp[i][j]`表示的状态为：考虑前```cpp i```号物品，使用不超过```cpp j```个单位的背包负重，能取得的最大价值

对于`dp[i][j]`这一状态来说，前`i - 1`号物品已经考虑完毕，此时第`i`号物品有选与不选两种选择，如果不选，那么使用的背包负重在考虑第`i`号物品前后并未改变，且背包中物品的最大价值相比只考虑前`i - 1`号物品时也没有改变，状态转移方程为`dp[i][j] = dp[i - 1][j]`；如果选，那么在考虑第`i`号物品后，使用的背包负重`j`中包含了第`i`号物品的重量`w[i]`，考虑之前使用的背包负重应为`j - w[i]`，最大价值相比考虑之前增加了第`i`号物品的价值`v[i]`，因此状态转移方程为`dp[i][j] = dp[i - 1][j - w[i]] + v[i]`

当背包空间`j < w[i]`时，不能选，当背包空间`j >= w[i]`时，可以选也可以不选，需要比较哪个情况更优，取较大值完成状态转移，完整的状态转移方程为
```cpp 
dp[i][j] = j < w[i] ? dp[i - 1][j]
    : std::max(dp[i - 1][j], dp[i - 1][j - w[i]] + v[i]);
```
初始状态：若背包负重为$0$，则显然最大价值为$0$，即`dp[i][0] = 0`；若不选任何物品，则最大价值也为$0$，即`dp[0][j] = 0`
==== 优化
...
在完全背包问题中，对于第`i`号物品也考虑选与不选两种情况，不选的情况下，状态转移方程与0-1背包问题中不选第$i$号物品的情况相同，为`dp[i][j] = dp[i - 1][j]`；选的情况下，如果第`i`号物品不是第一次被选，那么状态转移方程为`dp[i][j] = dp[i][j - w[i]] + v[i]`，而不是`dp[i][j] = dp[i - 1][j - w[i]] + v[i]`，因为此状态之前第`i`号物品已经被纳入考虑范围，如果是第一次选，那么应该让`dp[i][j - w[i]] + v[i]`与`dp[i - 1][j - w[i]] + v[i]`是同一个值，因为在第一次选之前背包容量显然处于`j < w[i]`的情况
