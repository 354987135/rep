...
1. 可以被分解为相同且规模更小的子问题
2. 具有*最优子结构*，即，问题的最优解可以由其子问题的最优解得到
3. 子问题具有重叠性，即，存在大量需要被重复计算的子问题
4. 状态系统具有无后效性，即，在已知当前状态的前提下，未来某个状态只与当前状态，以及该未来状态所处阶段有关，与过去状态无关，也就是说，过去状态和未来状态不会相互影响

  动态规划要求按顺序且不重复地计算每一个状态，因此必须满足无后效性

  以一个具体的例子来说明无后效性：
  
  $y$已在点$P$，并与$x$约定在点$P$会合，并一起到达点$Q$，$x$到达点$P$之前，$y$会无限等待$x$，且$x$不会独自去点$Q$，$x$有很多条路线可以到达点$P$，每一条不同的路线都会使$x$经过不同的点$A, B, C...$，但只要$x$选择其中一条路线，就一定可以到达点$P$
  
  在这一场景中，$x, y$能否共同到达点$Q$，与$x$是否到达点$A, B, C...$无关，只取决于$x$是否到达$P$点，这就是无后效性的体现
== 具体例子
问题：求斐波那契数列的第$n$项
...
===== 状态分析
状态`dp[i][j]`表示：考虑`i`号及其之前的所有物品，使用不超过```cpp j```个单位的背包负重，能取得的最大价值
