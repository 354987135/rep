...
+ 位数处理
    ...
    两个至多$k$位的正整数相加，和的位数至多为$k + 1$位，因此，$a n s$数组的长度应当为$k + 1$，$a, b$数组的长度应当统一为$k$，初始化代码见(_@大整数的表示[]_)
...
+ 位数处理
    ...
    两个至多$k$位的正整数相减，差的位数至多为$k$位，因此，$a n s, a, b$数组的长度都应当统一为$k$，初始化代码见(_@大整数的表示[]_)

+ 运算过程

    在减法竖式中，要保证被减数大于等于减数，因此首先要考虑$a, b$的大小关系(_@比较大整数[]_)，如果$a < 
    ...
    在这一过程中会出现$a n s_i < 0$的情况，得到形如$n = 1 space 3 space -23 space 8$的数，其中$n_1 = -23$，和加法(_@大整数加法[]_)类似，它等价于$n' = 1 space 0 space 7 space 8$，因为$-23 times 10^1 = -3 times 10^2 + 7 times 10^1 $，将$-3$加到$n_2$上，就可以得到$n'$，不难看出，借位相当于负的进位，因此，当$n_i < 0$时，$n_i$到$n'_i$的转换和$n_i > 9$的情况是相同的，减法和借位也可以分开处理
...
+ 位数处理
    ...
    一个$m$位的正整数与一个$n$位的正整数相乘，积的位数至多为$m + n$位，因此，$a n s$数组的长度应当为$m + n$，$a, b$数组的长度应当为各自的位数$m, n$，初始化代码见(_@大整数的表示[]_)
+ 运算过程
...
+ 位数处理
    ...
    一个$m$位的正整数除以一个$n$位的正整数，商的位数至多为$m - n + 1$位，余数的位数至多为$n$位，实际$a, b, q, r$数组的长度需要逐步分析，见(2. 运算过程)，初始化代码见(_@大整数的表示[]_)
