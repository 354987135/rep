...
+ 位数处理
// 修改LessThan 函数
// Transform 函数返回值是默认可移动实体，调用移动构造
商的位数至多为$m - n + 1$位，但在计算除法竖式时，我们需要进行$m$次试商，因此我们可以认为商的位数至多是$m$位
+ 运算过程
假设$b$不为$0$，模拟除法竖式，由于$a, b$都是使用数组存储的大数，我们不能直接进行试商，我们知道，商表示的就是被除数中至多能拆分出的除数的个数，显然，在被除数大于等于除数的前提下，我们可以不断从被除数中减去除数，这样的减法能够进行的次数就是该位的商，假设$a, b$从一开始就满足$a < b$，那么很明显，此时$r = a$，我们很容易想到，可以使用$r$去保存试第$i$位商时使用的$0 - i$位被除数，若$r < b$，则在第$i$位上商$0$，并将被除数的第$i + 1$位追加到$r$尾部
+ 输出处理
...
int main() {
    ...
    std::vector<int> a = Transform(aStr), b = Transform(bStr), q(aStr.size()), r(bStr.size());
