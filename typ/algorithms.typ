...
== 前缀和
前缀和即数组前$n$项的和，用于降低“反复求数组区间和”类问题的时间复杂度

以一维数组```cpp a ```的前缀和 (简称“一维前缀和”) 为例，前缀和```cpp p[i] ```定义为$ p_i = sum_(k = 1)^(i)a_k $

递推公式为```cpp p[i] = p[i - 1] + a[i] ```

因此，前缀和数组```cpp p ```所需的额外空间与数组```cpp a ```所占空间相同，创建```cpp p```的时间复杂度为$O(n)$

若要求$S = sum_(k = l)^(r)a_k$的值，根据一维前缀和的定义可得```cpp S = p[r] - p[l - 1] ```，求值时间复杂度为$O(1)$

若```cpp a ```为二维数组，那么```cpp a[i][j] ```的前缀和```cpp p[i][j] ```定义为$ p_(i j) = sum_(k_1 = 1)^(i)sum_(k_2 = 1)^(j)a_(k_1 k_2) $

递推公式为```cpp p[i][j] = p[i][j - 1] + p[i - 1][j] - p[i - 1][j - 1] + a[i][j] ```(画图可知)

若要求$S = sum_(k_1 = x_1)^(x_2)sum_(k_2 = y_1)^(y_2)a_(k_1 k_2)$的值，由图2可知，

```cpp S = p[x2][y2] - p[x1][y2] - p[x2][y1] + p[x1][y1] ```，求值时间复杂度为$O(1)$
== 差分
差分即数组相邻两项的差，用于降低“反复在数组区间上整体加$x$，并且只要求得到最终结果”类问题的时间复杂度

以一维数组```cpp a ```的差分 (简称“一维差分”) 为例，差分数组中```cpp d[i] ```定义为$d_i  = a_i - a_(i - 1)$，因此，差分数组```cpp d ```所需的额外空间与数组```cpp a ```相同，创建```cpp d ```的时间复杂度为$O(n)$

根据一维差分的定义，我们可以发现

$ sum_(k = 1)^i d_i = a_1 + (a_2 - a_1) + (a_3 - a_2) + ... + (a_i - a_(i - 1)) = a_i $

即原数组```cpp a ```是差分数组```cpp d ```的前缀和数组，若将一维前缀和递推式中的```cpp p, a```数组分别看作```cpp a, d```数组，进行移项即可得到差分数组中```cpp d[i] ```的定义

此外，还可以得到一个非常有用的性质：如果对$d_i$加常数$C$再更新数组```cpp a ```，那么就相当于对从$a_i$开始往后的所有项都加$C$，若要对$a_l$到$a_r$的每一项执行加$C$操作，只需要做$d_l + C$和$d_(r + 1) - C$，然后更新数组```cpp a ```

利用这一性质，可以先完成多次区间修改操作，再更新数组```cpp a ```，从而降低时间复杂度

若```cpp a ```为二维数组，将二维前缀和递推式中的，差分```cpp d[i][j] ```定义为