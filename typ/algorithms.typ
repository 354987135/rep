...
= 基础算法
== 目录
- C++语言基础核心特性（只需要知道这些特性的存在，并且知道如何使用这些特性，而不需要深究其中的数据结构与算法细节）
- 基础数据结构概览（描述一些基础数据结构（普及组）的定义、形态、及利用C++语言定义该数据结构类，简要探讨相关算法，不涉及细节）
- 基础算法（普及组篇）
    + 模拟
    + 枚举（蛮力法） （可能安排包括搜索）
    + 二分、倍增
...
== 模拟 (Implementation)
对于一些无法应用任何算法的题目，只能使用代码模拟问题的场景和过程并从中得到解，这样的解题方法称为模拟法
== 递归
=== 计算机中的函数与数学函数的区别
...
```
=== 深度优先搜索
若问题的解之间存在状态转移关系，那么将解抽象为节点、关系抽象为边后，整个解空间就可以看成一棵有根树，通过递归从树根开始枚举并检查每个解是否可行的操作就称为深度优先搜索 (DFS)

DFS的本质是暴力枚举，时间复杂度差，通常我们需要通过剪枝、记忆化等方法来优化，

相较于利用循环枚举来说，递归枚举具有代码简洁、适用范围广的优势，在使用循环枚举时我们需要明确知道具体的解，而搜索时只需要知道解的初始情况和最终情况，中间的所有解都会由递归函数自动生成
=== 回溯法
回溯法是DFS能够枚举所有解的关键步骤，其代表的含义是：在探索完解空间（树）某一条分支的一个叶节点后，应当退回前一个状态，使得它能够探索其他的叶节点，因此回溯的操作应该发生在探索完一个完整的分支后，了解这个原则可以帮助我们编写DFS代码
=== 分治法
==== 分治算法的时间复杂度
==== 主定理 ( Master Theorem )
设时间复杂度为 $T(n)$，则 
...
+ 大整数乘较小数时的优化
    ...
    确定一个整数的位数可计算其以$10$为底的对数，然后取整再$+1$
    ...
    std::vector<int> Multiply(const std::vector<int>& a, int b) {
        std::vector<int> ans(a.size() + int(log10(b)) + 1);
        for (int i = 0; i < a.size(); ++i) {
            ans[i] = a[i] * b;
        }
        return ans;
...
public:
    ...
    friend BigInt operator*(const BigInt& a, int b) {
        ...
        b = std::abs(b);
        ans.num.resize(a.num.size() + int(log10(b)) + 1);
