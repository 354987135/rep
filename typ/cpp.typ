...
注意，由于使用到了算术运算符```cpp - ```，必须确保绝对值在```cpp int ```可表示的范围内，否则如果发生有符号整数溢出，则程序行为未定义
...
=== 用位运算实现整数四则运算
==== 加法
设$8$位整数$a = 01001011, b = 00111010$，通过观察可以发现，$a arrowhead.t b$的结果是$a,b$无进位加法的结果，而$a space \& space b$的结果是$a, b$加法的进位信息，例如
$ 
...
运算时不需要区分正负数，这一点由补码的性质(*_@有符号整数的编码[]_*)保证
...
```
==== 乘法 <位运算实现乘法>
设$8$位整数$a = 11011011, b = 10110001$，则$a * b$的竖式运算过程如下
...
由(*_@按位右移[]_*)，可知，对负数执行按位右移运算会在左侧补$1$，即当$b < 0$时，无法形成循环终止条件$b = 0$，为了处理这一情况，在右移之后还需要将最高位设置为$0$，该操作的结果可以通过$b space \& space m a s k$得到，其中$m a s k = 0111...1111$
...
```
==== 除法
以$8$位整数$a = 11011011, b = 10110001, a n s = 01101011$为例，通过(*_@位运算实现乘法[]_*)的乘法运算，我们知道$a n s = a * b = a << 0 + a << 4 + a << 5 + a << 7$，其中的$0, 4, 5, 7$代表的含义是$b$的第$0, 4, 5, 7$位上是$1$，那么假设现在已知$a n s, a$，利用$a n s$和$a$按从大到小的顺序找出$7, 5, 4, 0$，然后得到$b$的过程就是$a n s div a$的算法实现

具体来说，我们需要重复寻找不超过$a n s$的最大的$a << i$，将$b_i$设为$1$，并从$a n s$中减去$a << i$，直到$a n s < a$

执行按位左移操作时，可能将$1$移到符号位上，导致结果发生错误，为了避免这个问题，可以将$a n s$与$a << i$的比较转换为$a n s >> i$与$a$的比较，由于补码不能直接进行大小比较，如果除法运算中出现负数，则需要先将其转换为绝对值才能执行该算法
...
int Divide(int a, int b) {
    int ans = 0;
    uint32_t ua = a >> 31 ? Add(~a, 1) : a;
    uint32_t ub = b >> 31 ? Add(~b, 1) : b;
    for (int i = 31; i >= 0; --i) {
        if ((ua >> i) >= ub) {
            ua = Subtract(ua, ub << i);
            ans |= 1 << i;
    ...
    return (a >> 31) ^ (b >> 31) ? Add(~ans, 1) : ans;
}
```
= 指针与数组
== 原始指针
