...
int Multiply(int a, int b) {
    int ans = 0, mask = ~(1 << sizeof(int) * 8 - 1);
    while (b) {
        if (b & 1) {
            ans = Add(ans, a);
        }
...
```
==== 除法
以$8$位整数$a = 11011011, b = 10110001, a n s = 01101011$为例，通过(*_@位运算实现乘法[]_*)的乘法运算，我们知道$a n s = a * b = a << 0 + a << 4 + a << 5 + a << 7$，其中的$0, 4, 5, 7$代表的含义是$b$的第$0, 4, 5, 7$位上是$1$，那么假设现在已知$a n s, a$，我们的任务就是利用$a n s$和$a$确定出$b$中是$1$的位是哪几位，这一求$b$的过程就是$a n s div a$的实现

在$a << i$中，确定最大的$i$比确定最小的$i$要容易，因此，我们应当按从大到小顺序依次求出$i$，在上面的例子中，要按$7, 5, 4, 0$的顺序找出每个$i$

具体来说，在求某个$i$时，将$a$不断左移，直到得到一个不超过$a n s$的最大数，此时左移的总次数就是我们要求的$i$，然后从$a n s$中减去$a << i$，将$b_i$设为$1$，重复此过程，直到$a n s < a$

由于需要进行比较运算，需要将被除数和除数都转为绝对值再执行上述算法
