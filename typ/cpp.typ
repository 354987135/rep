...
+ 无符号整数不存在负数，因此可以直接使用二进制数值表示，例如，要将$55$用$8$位二进制数表示，则先将$55$转为二进制，得到$110111$，再在高位用$0$填补不足的位，就可以得到$55$的$8$位二进制表示$00110111$

+ 有符号整数的编码

  + 有符号整数由于符号的存在，编码时需要考 虑符号信息的表示，目前被广泛应用的编码方式有$4$种，分别为原码(_sign-magnitude_)，反码(_one's complement_)，补码(_two's complement_)，移码(_offset binary_)

  + 原码类似无符号整数的二进制表示，只是将最高位保留为符号位，用$0$表示非负数，用$1$表示负数，反码和补码由原码变换而来，用于解决有负数参与的加减法问题，因此，反码和补码是针对负数的，非负数的原码，反码，补码都是同一个二进制数，移码由补码变换而来，用于表示浮点数，此处不展开讨论这些问题

  + 设$x$为非负数，则$-x$的$k$位反码表示为$2^k - 1 - x$，即$underbrace(1...1, "k个1") - x$
  
    简化算法：保持原码的符号位不变，取反全部数值位，从反码到原码的转换方法相同

  + 设$x$为非负数，则$-x$的$k$位补码表示为$2^k - x$

    简化算法：保持原码的符号位不变，取反全部数值位并在末尾$+1$，忽略最高位的进位，从补码到原码的转换方法相同

  + 针对补码的硬件最容易设计，因此绝大多数计算机系统中使用补码表示整数
== 位运算的注意点
+ 位运算的操作数都是整数，能够参与位运算的具体类型详见#link("https://en.cppreference.com/w/cpp/language/operator_arithmetic")[*_Built-in bitwise logic operators_*]和#link("https://en.cppreference.com/w/cpp/language/operator_arithmetic")[*_Built-in bitwise shift operators_*]
