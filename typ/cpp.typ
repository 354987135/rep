...
=== 无符号整数的编码
无符号整数不存在负数，因此可以直接使用二进制数值表示，例如，要将$55$用$8$位二进制数编码，则先将$55$转为二进制，得到$110111$，再在高位用$0$填补不足的位，就可以得到$55$的$8$位二进制编码$00110111$
=== 有符号整数的编码
...
+ 反码和补码由原码变换而来，用于解决有负数参与的加减法问题，因此，反码和补码是针对负数的，非负数的原码，反码，补码都是同一个二进制数，绝大多数计算机系统中都使用补码表示整数

  设$x$为非负数，则$-x$的$k$位反码表示为$2^k - 1 - x$，即$underbrace(1...1, "k个1") - x$，相当于保持原码的符号位不变，取反全部数值位，该方法也可以完成从反码到原码的转换

  设$x$为非负数，则$-x$的$k$位补码表示为$2^k - x$，相当于保持原码的符号位不变，取反全部数值位并在最低位$+1$，并忽略最高位的进位，该方法也可以完成从补码到原码的转换
