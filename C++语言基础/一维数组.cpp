...
// 必须时刻注意
 ...
 // 数组中 插入, 删除元素
    ...
    // 变成 1 3 5 9 6 2
    // 注意点:
    // 1. 需要在原数组上修改, 而不是创建新数组
    // 2. 数组的长度必须大于插入之后元素的个数
    // 3. 两个重要操作: 插入, 移动
  // int a[10] {1, 3, 5, 6, 2};
  // int N = 5;
  // int index = 3;
  ...
  // cout << sizeof a;
  // 2. 查询某类型的变量会占用多少内存空间
  // 查看 int 类型的变量会占用多少内存空间
  // cout << sizeof(double);

  // int a[10];
  // cout << sizeof a;
//sizeof 数组变量名 = 数组的长度 * sizeof(数组元素类型)
  ...
  // int a[] {8, 2, 3, 4, 7, 5, 1};
  // int b[] {-1, -3, -4, -5};
  // 题目要求：找出 数组a 中的最大值和最小值，
  // 还有它们对应的下标
  // 方法：假设某个值是最大值或最小值
  // 假设 max 是最大值，min 是最小值
  // 因此必须将 max、min 初始化
  // 初始化的要求：
  // 必须要让 max 的初始值小于数组中的所有数
  // 必须要让 min 的初始值大于数组中的所有数
  // 通常题目会给出数据范围，根据数据范围选择就好
  // 如果可以的话，让max、min 初始值都为 a[0] 也可以
  // int max = 0, min = 10, maxi, mini;
  // for (int i = 0; i < 7; ++i) {
  //   if (max < a[i]) {
  //     max = a[i];
  //     maxi = i;
  ...
  //   }
  // }
  // cout << "最大值：" << max << " 下标：" << maxi << '\n';
  // cout << "最小值：" << min << " 下标：" << mini << '\n';
...
//     3, 4, 2, 1, 1, 2, 4, 2};
  // 题目要求：统计出这些数字的出现次数
  // 额外要求：不创建更多变量，只使用 数组a 来统计
  // 对于整数来说，数组的下标可以用来标记整数
  // a[1] 可以表示 数字 1 出现的次数
  // 这些位置叫做“桶”
  // 这样就可以不用创建很多变量，
  // 使用一个数组就可以统计出所有整数的出现次数
